...
function AuthPage() {
  ...
  async function google() {
    ...
    try {
      ...
      // in your Supabase project dashboard (with its own Google OAuth client ID/secret).
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: { redirectTo: window.location.origin + "/dashboard" },
      });
      if (error) throw error;
      // On success Supabase redirects the browser to Google itself; nothing more to do here.
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-2xl">
          <div className="flex gap-2 mb-6 rounded-lg bg-muted p-1">
            <button onClick={() => setMode("login")} className={`flex-1 rounded-md px-3 py-1.5 text-sm ${mode === "login" ? "bg-background shadow" : "text-muted-foreground"}`}>{t("auth.login")}</button>
            <button onClick={() => setMode("signup")} className={`flex-1 rounded-md px-3 py-1.5 text-sm ${mode === "signup" ? "bg-background shadow" : "text-muted-foreground"}`}>{t("auth.signup")}</button>
          </div>
          ...
          <div className="relative mb-4"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-border" /></div><div className="relative flex justify-center text-xs"><span className="bg-card px-2 text-muted-foreground">{t("auth.or")}</span></div></div>
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div>
              ...
              <input type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder={t("auth.passwordHint")} />
            </div>
